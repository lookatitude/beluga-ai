...
InitMetrics initializes the global metrics instance for Turn Detection operations. This should be called once during application startup to enable metrics collection.
...
NewProvider creates a new Turn Detection provider instance based on the configuration. It uses the global registry to find and instantiate the appropriate provider. Turn Detection identifies when a speaker has finished speaking and it's time for a response. Supported providers include: silence-based, energy-based, etc.
