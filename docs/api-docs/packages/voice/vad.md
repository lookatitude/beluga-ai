...
InitMetrics initializes the global metrics instance for VAD operations. This should be called once during application startup to enable metrics collection.
...
NewProvider creates a new VAD provider instance based on the configuration. It uses the global registry to find and instantiate the appropriate provider. VAD (Voice Activity Detection) identifies when speech is present in audio streams. Supported providers include: webrtc, silero, etc.
