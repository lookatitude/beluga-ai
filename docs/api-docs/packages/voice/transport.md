...
InitMetrics initializes the global metrics instance for Transport operations. This should be called once during application startup to enable metrics collection.
...
NewProvider creates a new Transport provider instance based on the configuration. It uses the global registry to find and instantiate the appropriate provider. Transport handles audio data transmission between components in the voice pipeline. Supported providers include: websocket, webrtc, etc.
