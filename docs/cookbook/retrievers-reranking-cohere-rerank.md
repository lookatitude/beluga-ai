...
type RerankedRetriever struct {
    retriever iface.Retriever
    reranker  *CohereReranker
    initialK  int // Retrieve more initially, then rerank to topN
}

// NewRerankedRetriever creates a new reranked retriever
func NewRerankedRetriever(retriever iface.Retriever, reranker *CohereReranker, initialK int) *RerankedRetriever {
...
1. **Two-stage retrieval** - Notice how we first retrieve more documents (initialK) than needed, then rerank to get the top results. This gives the reranker more candidates to choose from, improving final quality.

2. **Reranking integration** - We wrap the base retriever with reranking functionality. This allows you to add reranking to any retriever without modifying the base implementation.

3. **Fallback on errors** - If reranking fails, we fall back to the original results. This ensures retrieval still works even if the rerank service is unavailable.
...
- **[Retrievers Parent Document Retrieval (PDR)](./retrievers-parent-document-retrieval.md)** - Retrieve parent documents
- **[Vectorstores Advanced Meta-filtering](./vectorstores-advanced-meta-filtering.md)** - Advanced filtering
