...
Implement clustering algorithms that incorporate metadata as constraints or features, allowing you to create clusters that are both semantically similar and metadata-coherent. This works because combining embedding similarity with metadata constraints produces clusters that are more useful for real-world applications.
...
func NewMetadataAwareClusterer(embedder iface.Embedder, metadataWeight float64, minClusterSize int) *MetadataAwareClusterer {
...
// findBestCluster finds the best cluster for a document
...
// metadataSimilarity calculates metadata similarity
...
func (mac *MetadataAwareClusterer) aggregateMetadata(docs []DocumentWithEmbedding) map[string]interface{} {
    ...
    // Take most common value for each key
...
func main() {
    ...
    clusterer := NewMetadataAwareClusterer(embedder, 0.3, 5) // 30% metadata weight, min 5 docs per cluster
...
1. **Combined similarity metric** - Notice how we combine embedding similarity with metadata similarity using a weight parameter. This allows you to balance semantic similarity with domain constraints (e.g., keep documents from the same category together).

2. **Metadata aggregation** - We aggregate metadata from cluster members to create cluster-level metadata. This helps identify what each cluster represents (e.g., "tech documents from 2024").

3. **Minimum cluster size** - We filter out clusters below a minimum size. This prevents creating too many tiny clusters that aren't useful for retrieval.

```go
**Key insight:** Metadata constraints help create clusters that respect domain boundaries. A document about "Python" in the "programming" category shouldn't cluster with "Python" the snake in the "animals" category, even if embeddings are similar.
...
```go
func TestMetadataAwareClusterer_RespectsMetadata(t *testing.T) {
    mockEmbedder := &MockEmbedder{}
    clusterer := NewMetadataAwareClusterer(mockEmbedder, 0.5, 2)
    
    documents := []schema.Document{
        schema.NewDocument("Python programming", map[string]string{"category": "tech"}),
        schema.NewDocument("Python snake", map[string]string{"category": "animals"}),
    }
    
    clusters, err := clusterer.ClusterDocuments(context.Background(), documents, 2)
    require.NoError(t, err)
    
    // With high metadata weight, should create separate clusters
...
- **[Embeddings Batch Embedding Optimization](./embeddings-batch-optimization.md)** - Optimize batch operations
