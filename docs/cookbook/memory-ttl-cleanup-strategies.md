---
title: "Memory TTL & Cleanup Strategies"
package: "memory"
category: "memory"
complexity: "intermediate"
---

# Memory TTL & Cleanup Strategies
...
Implement a TTL-based cleanup system that tracks memory creation times, periodically checks for expired entries, and removes them while preserving recent and important context. This works because you can associate timestamps with memory entries and run background cleanup processes.
...
// MemoryEntry represents a memory entry with metadata
...
func NewTTLMemoryWrapper(memory iface.Memory, ttl time.Duration, cleanupInterval time.Duration) *TTLMemoryWrapper {
...
func (tmw *TTLMemoryWrapper) LoadMemoryVariables(ctx context.Context, inputs map[string]interface{}) (map[string]interface{}, error) {
    ...
    if exists {
        ...
        if time.Now().After(entry.ExpiresAt) {
            ...
            // Still try to load from underlying memory (it may have its own expiration)
...
func (tmw *TTLMemoryWrapper) cleanupExpired(ctx context.Context) {
    ...
    for _, key := range expired {
        ...
        // Optionally clean from underlying memory
        // This depends on memory implementation
    }
    ...
    if len(expired) > 0 {
        span.SetStatus(trace.StatusOK, fmt.Sprintf("cleaned up %d expired entries", len(expired)))
    } else {
        span.SetStatus(trace.StatusOK, "no expired entries")
    }
}
...
func main() {
    ...
    // Create TTL wrapper
    // memory := yourMemory
    ttlMemory := NewTTLMemoryWrapper(memory, 24*time.Hour, 1*time.Hour)
    ...
    // Load will automatically check TTL
...
1. **TTL tracking** - Notice how we track when each memory entry was created and when it expires. This allows us to automatically identify and remove expired entries.

2. **Periodic cleanup** - We run a background cleanup loop that periodically checks for expired entries and removes them. This prevents memory from growing unbounded.

3. **Access-based extension** - When entries are accessed, we can extend their TTL. This keeps frequently-used memory alive while removing unused entries.

```go
**Key insight:** Use TTL for automatic cleanup, but allow priority-based extensions for important conversations. This balances memory efficiency with preserving valuable context.
...
```go
func TestTTLMemoryWrapper_ExpiresEntries(t *testing.T) {
    mockMemory := &MockMemory{}
    ttlMemory := NewTTLMemoryWrapper(mockMemory, 1*time.Second, 100*time.Millisecond)
    defer ttlMemory.Stop()
    
    // Save entry
    inputs := map[string]interface{}{"session_id": "test"}
    outputs := map[string]interface{}{"data": "value"}
    ttlMemory.SaveContext(context.Background(), inputs, outputs)
...
Check expiration only on access:
...
- **[Memory Recipes](./memory-recipes.md)** - Basic memory patterns
