...
func (rr *RetryRunnable) Batch(ctx context.Context, inputs []any, options ...core.Option) ([]any, error) {
    ...
    // Retry the entire batch if it fails
...
func (rr *RetryRunnable) isRetryable(err error) bool {
    ...
    // Check for context timeout (don't retry)
    if errors.Is(err, context.DeadlineExceeded) || errors.Is(err, context.Canceled) {
        return false
    }
    
    // Default: retry on any error (can be customized)
    return true
}
...
1. **Runnable interface implementation** - Notice how `RetryRunnable` implements all three `Runnable` methods (`Invoke`, `Batch`, `Stream`). This allows it to wrap any runnable transparently - the caller doesn't need to know retry logic is applied.
...
3. **Context awareness** - We check `ctx.Err()` before each attempt and during backoff. This ensures we respect timeouts and cancellations from the calling code, preventing wasted retries.

```go
**Key insight:** Retry wrappers should be composable. You can wrap a retry wrapper with tracing, or wrap multiple layers for different retry strategies per component type.
...
- **[Core Advanced Context Timeout Management](./core-advanced-context-timeout-management.md)** - Advanced timeout handling
- **[LLM Error Handling](./llm-error-handling.md)** - LLM-specific error handling with retries
- **[Core Package Guide](../package_design_patterns.md)** - For a deeper understanding of Runnable composition
