...
# Token Counting without Performance Hit
...
// FastEstimator estimates tokens using character count (very fast)
...
func main() {
    ...
    // Create token counter with exact counter and estimator
    ...
    // Count tokens (uses estimation for speed)
...
1. **Caching** - Notice how we cache token counts by text content. Repeated inputs (like common prompts) are counted once and cached, eliminating redundant counting. This is important because many applications reuse the same prompts or system messages.

2. **Estimation fallback** - When exact counts aren't required (e.g., for rate limiting), we use fast estimation. The exact count is computed asynchronously and cached for future use. This gives you immediate results without blocking.

3. **Async counting** - For non-blocking scenarios, we provide async counting that returns an estimated count immediately and the exact count later. This allows your application to proceed while accurate counts are computed in the background.

```go
**Key insight:** Use estimation for rate limiting and caching for exact counts. Most use cases don't need exact counts immediately - they can be computed asynchronously and used for billing/analytics later.
...
- **[LLMs Streaming Tool Logic Handler](./llms-streaming-tool-logic-handler.md)** - Handle streaming with tool calls
- **[Monitoring Custom Metrics for S2S Voice](./monitoring-custom-metrics-s2s-voice.md)** - Track token usage metrics
- **[LLMs Package Guide](../guides/llm-providers.md)** - For a deeper understanding of LLM operations
