...
func NewStreamingToolHandler(toolList []tools.Tool) *StreamingToolHandler {
...
func (sth *StreamingToolHandler) HandleStreamingWithTools(ctx context.Context, model iface.ChatModel, messages []schema.Message, toolList []tools.Tool) (<-chan schema.Message, error) {
...
func (sth *StreamingToolHandler) executeTool(ctx context.Context, toolCall schema.ToolCall) {
    ...
    tool, exists := sth.tools[toolCall.Name]
    if !exists {
        result := ToolResult{
...
func main() {
    ...
    // Create model and messages
    // model := yourModel
    // messages := []schema.Message{...}
    
    // Handle streaming with tools
    // outputCh, err := handler.HandleStreamingWithTools(ctx, model, messages, toolList)
...
1. **Incremental tool call collection** - Notice how we accumulate tool call chunks as they arrive. Tool calls may be split across multiple chunks, so we buffer the arguments until we receive a chunk marked as complete.
...
```go
**Key insight:** Start tool execution as soon as you have enough information, not when the stream completes. This reduces perceived latency and improves user experience.
...
func TestStreamingToolHandler_ExecutesTools(t *testing.T) {
    ...
    handler := NewStreamingToolHandler([]tools.Tool{tool})
...
- **[LLMs Token Counting without Performance Hit](./llms-token-counting-no-performance-hit.md)** - Optimize token counting
- **[Agents Handling Tool Failures & Hallucinations](./agents-handling-tool-failures-hallucinations.md)** - Robust tool error handling
- **[LLMs Package Guide](../guides/llm-streaming-tool-calls.md)** - For a deeper understanding of streaming
