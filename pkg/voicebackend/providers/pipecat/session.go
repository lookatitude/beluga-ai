...
func NewPipecatSession(config *PipecatConfig, sessionConfig *vbiface.SessionConfig, roomName string, httpClient *http.Client) (*PipecatSession, error) {
	sessionID := uuid.New().String()

	orchestrator := internal.NewPipelineOrchestrator(config.Config)

	return &PipecatSession{
		id:                   sessionID,
		roomName:             roomName,
		config:               config,
...
func (s *PipecatSession) GetID() string {
	return s.id
}

// Start starts the voice session, connecting to Daily.co room via WebSocket.
func (s *PipecatSession) Start(ctx context.Context) error {
	s.mu.Lock()
	defer s.mu.Unlock()

	if s.active {
		return voicebackend.NewBackendError("Start", "session_already_active", nil)
	}
	...
	// 1. Connect to Daily.co room via WebSocket
...
// Stop stops the voice session, disconnecting from Daily.co room.
func (s *PipecatSession) Stop(ctx context.Context) error {
	s.mu.Lock()
	defer s.mu.Unlock()

	if !s.active {
		return voicebackend.NewBackendError("Stop", "session_not_active", nil)
	}
	...
	// 4. Optionally delete Daily.co room

	return nil
}

// ProcessAudio processes incoming audio data through the pipeline orchestrator.
func (s *PipecatSession) ProcessAudio(ctx context.Context, audio []byte) error {
	ctx, span := voicebackend.StartSpan(ctx, "PipecatSession.ProcessAudio", "pipecat")
	defer span.End()
	...
	if !active {
		err := voicebackend.NewBackendError("ProcessAudio", "session_not_active", nil)
	...
	if err != nil {
		...
		if voicebackend.IsRetryableError(err) {
			voicebackend.LogWithOTELContext(ctx, slog.LevelWarn, "Retryable error in audio processing, attempting recovery", "error", err)
			...
			case <-time.After(100 * time.Millisecond):
				...
				if err != nil {
					...
					s.mu.Unlock()
					voicebackend.RecordSpanError(span, err)
					voicebackend.LogWithOTELContext(ctx, slog.LevelError, "Failed to process audio after retry", "error", err)
					return err
				}
				voicebackend.LogWithOTELContext(ctx, slog.LevelInfo, "Audio processing recovered from transient error")
			}
		} else {
			// Non-retryable error
			s.mu.Lock()
			...
			s.mu.Unlock()
			voicebackend.RecordSpanError(span, err)
			voicebackend.LogWithOTELContext(ctx, slog.LevelError, "Failed to process audio", "error", err)
			return err
		}
	}
	...
	case <-time.After(5 * time.Second):
		...
		voicebackend.LogWithOTELContext(ctx, slog.LevelWarn, "Audio output buffer full, dropping frame",
	...
	voicebackend.LogWithOTELContext(ctx, slog.LevelInfo, "Audio processed successfully",
		"input_size", len(audio), "output_size", len(outputAudio))

	return nil
}

// SendAudio sends audio data to the user via Daily.co WebSocket.
func (s *PipecatSession) SendAudio(ctx context.Context, audio []byte) error {
	s.mu.RLock()
	defer s.mu.RUnlock()

	if !s.active {
		return voicebackend.NewBackendError("SendAudio", "session_not_active", nil)
	}

	// TODO: Implement WebSocket send to Daily.co room
	...
	case <-ctx.Done():
		return ctx.Err()
	case <-time.After(5 * time.Second):
		return voicebackend.NewBackendError("SendAudio", voicebackend.ErrCodeTimeout, nil)
	}
}

// ReceiveAudio receives audio data from the user via Daily.co WebSocket.
func (s *PipecatSession) ReceiveAudio() <-chan []byte {
	return s.audioOutput
}

// SetAgentCallback sets the agent callback function.
func (s *PipecatSession) SetAgentCallback(callback func(context.Context, string) (string, error)) error {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *PipecatSession) SetAgentInstance(agent agentsiface.Agent) error {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *PipecatSession) GetState() vbiface.PipelineState {
	s.mu.RLock()
	defer s.mu.RUnlock()
	return s.state
}
...
func (s *PipecatSession) GetPersistenceStatus() vbiface.PersistenceStatus {
	s.mu.RLock()
	defer s.mu.RUnlock()
...
func (s *PipecatSession) UpdateMetadata(metadata map[string]any) error {
	s.mu.Lock()
	defer s.mu.Unlock()
