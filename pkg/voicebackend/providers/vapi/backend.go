...
// VapiBackend implements the VoiceBackend interface for Vapi.
type VapiBackend struct {
	config          *VapiConfig
...
// NewVapiBackend creates a new Vapi voicebackend.
func NewVapiBackend(config *VapiConfig) (*VapiBackend, error) {
...
// Start starts the Vapi backend, initializing the connection.
func (b *VapiBackend) Start(ctx context.Context) error {
	ctx, span := voicebackend.StartSpan(ctx, "VapiBackend.Start", "vapi")
	defer span.End()
	...
	if err != nil {
		...
		voicebackend.LogWithOTELContext(ctx, slog.LevelError, "Failed to connect to Vapi API", "error", err)
	...
	voicebackend.LogWithOTELContext(ctx, slog.LevelInfo, "Vapi backend started successfully")
	return nil
}

// Stop stops the Vapi backend gracefully.
func (b *VapiBackend) Stop(ctx context.Context) error {
	ctx, span := voicebackend.StartSpan(ctx, "VapiBackend.Stop", "vapi")
	defer span.End()
	...
	voicebackend.LogWithOTELContext(ctx, slog.LevelInfo, "Vapi backend stopped successfully")
	return nil
}

// CreateSession creates a new voice session by creating a Vapi call.
func (b *VapiBackend) CreateSession(ctx context.Context, config *iface.SessionConfig) (iface.VoiceSession, error) {
	startTime := time.Now()
	ctx, span := voicebackend.StartSpan(ctx, "VapiBackend.CreateSession", "vapi")
	defer span.End()

	// Track concurrent operations
	if b.metrics != nil {
		b.metrics.IncrementConcurrentOps(ctx, "vapi")
		defer b.metrics.DecrementConcurrentOps(ctx, "vapi")
	}
	...
	err := internal.RetryWithBackoff(ctx, retryConfig, "VapiBackend.CreateSession", func() error {
		...
		// TODO: Add call configuration (assistant_id, phone_number_id, etc.)
	...
	if err != nil {
		voicebackend.RecordSpanError(span, err)
		voicebackend.LogWithOTELContext(ctx, slog.LevelError, "Failed to create Vapi call", "error", err)
		return nil, voicebackend.WrapError("CreateSession", err)
	}

	// Create Vapi session
	session, err := NewVapiSession(b.config, config, b.httpClient)
	if err != nil {
		voicebackend.RecordSpanError(span, err)
		return nil, voicebackend.WrapError("CreateSession", err)
	}
	...
	if b.metrics != nil {
		b.metrics.RecordSessionCreationTime(ctx, "vapi", creationTime)
	}
	voicebackend.AddSpanAttributes(span, map[string]any{
	...
	voicebackend.LogWithOTELContext(ctx, slog.LevelInfo, "Vapi session created successfully",
		"session_id", session.GetID(), "creation_time_ms", creationTime.Milliseconds())
	return session, nil
}

// GetSession retrieves a voice session by ID.
func (b *VapiBackend) GetSession(ctx context.Context, sessionID string) (iface.VoiceSession, error) {
	return b.sessionManager.GetSession(sessionID)
}

// ListSessions returns all active voice sessions.
func (b *VapiBackend) ListSessions(ctx context.Context) ([]iface.VoiceSession, error) {
	return b.sessionManager.ListSessions(), nil
}

// CloseSession closes a voice session.
func (b *VapiBackend) CloseSession(ctx context.Context, sessionID string) error {
	return b.sessionManager.CloseSession(ctx, sessionID)
}

// HealthCheck checks the health status of the voicebackend.
func (b *VapiBackend) HealthCheck(ctx context.Context) (*iface.HealthStatus, error) {
	b.mu.RLock()
	defer b.mu.RUnlock()
...
func (b *VapiBackend) GetConnectionState() iface.ConnectionState {
	b.mu.RLock()
	defer b.mu.RUnlock()
