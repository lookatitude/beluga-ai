...
func (s *LiveKitSession) HandleConnectionLoss(ctx context.Context) error {
	...
	voicebackend.LogWithOTELContext(ctx, slog.LevelWarn, "WebRTC connection lost, attempting reconnection",
...
func (s *LiveKitSession) reconnect(ctx context.Context) {
	...
	for attempt := 0; attempt < maxAttempts; attempt++ {
		...
		case <-time.After(retryDelay):
			...
			if s.active {
				...
				voicebackend.LogWithOTELContext(ctx, slog.LevelInfo, "WebRTC connection reestablished",
	...
	s.mu.Unlock()
	voicebackend.LogWithOTELContext(ctx, slog.LevelError, "Failed to reconnect WebRTC connection after max attempts",
...
func (s *LiveKitSession) Stop(ctx context.Context) error {
	...
	if s.config.DataRetentionHook != nil {
		...
		if err != nil {
			voicebackend.LogWithOTELContext(ctx, slog.LevelError, "Data retention hook error", "error", err, "session_id", s.id)
			// Continue with default behavior on error
		} else if shouldRetain {
			s.persistenceStatus = iface.PersistenceStatusActive
			voicebackend.LogWithOTELContext(ctx, slog.LevelInfo, "Session data will be retained",
				"session_id", s.id)
		} else {
...
func (s *LiveKitSession) ProcessAudio(ctx context.Context, audio []byte) error {
	ctx, span := voicebackend.StartSpan(ctx, "LiveKitSession.ProcessAudio", "livekit")
	defer span.End()
	...
	if connState != iface.ConnectionStateConnected {
		...
		if err := s.HandleConnectionLoss(ctx); err != nil {
			return voicebackend.WrapError("ProcessAudio", err)
		}
		// Return error to indicate connection issue
		return voicebackend.NewBackendError("ProcessAudio", voicebackend.ErrCodeConnectionFailed,
...
func (s *LiveKitSession) UpdateMetadata(metadata map[string]any) error {
	...
	if s.roomService != nil {
		...
		if err != nil {
			return voicebackend.WrapError("UpdateMetadata", err)
		}
	}
