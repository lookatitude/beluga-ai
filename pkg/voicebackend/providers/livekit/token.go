...
func GenerateAccessTokenWithHook(ctx context.Context, config *LiveKitConfig, roomName, participantIdentity string, authHook iface.AuthHook) (string, error) {
	...
	if authHook != nil && config.AuthHook != nil {
		...
		if err != nil {
			return "", voicebackend.NewBackendError("GenerateAccessTokenWithHook", voicebackend.ErrCodeAuthenticationFailed, err)
		}

		if !authResult.Authorized {
			return "", voicebackend.NewBackendError("GenerateAccessTokenWithHook", voicebackend.ErrCodeAuthorizationFailed,
				nil)
		}
