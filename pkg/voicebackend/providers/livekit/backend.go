...
import (
	...
	lksdkwrapper "github.com/lookatitude/beluga-ai/pkg/voicebackend/providers/livekit/internal"
...
// NewLiveKitBackend creates a new LiveKit voicebackend.
...
func (b *LiveKitBackend) Start(ctx context.Context) error {
	ctx, span := voicebackend.StartSpan(ctx, "LiveKitBackend.Start", "livekit")
	defer span.End()
	...
	if err != nil {
		...
		voicebackend.LogWithOTELContext(ctx, slog.LevelError, "Failed to create LiveKit room service client", "error", err)
	...
	voicebackend.LogWithOTELContext(ctx, slog.LevelInfo, "LiveKit backend started successfully")
	return nil
}
...
func (b *LiveKitBackend) Stop(ctx context.Context) error {
	ctx, span := voicebackend.StartSpan(ctx, "LiveKitBackend.Stop", "livekit")
	defer span.End()
	...
	voicebackend.LogWithOTELContext(ctx, slog.LevelInfo, "LiveKit backend stopped successfully")
	return nil
}
...
func (b *LiveKitBackend) CreateSession(ctx context.Context, config *iface.SessionConfig) (iface.VoiceSession, error) {
	startTime := time.Now()
	ctx, span := voicebackend.StartSpan(ctx, "LiveKitBackend.CreateSession", "livekit")
	defer span.End()
	...
	if b.config.AuthHook != nil {
		...
		if err != nil {
			voicebackend.RecordSpanError(span, err)
			return nil, voicebackend.NewBackendError("CreateSession", voicebackend.ErrCodeAuthenticationFailed, err)
		}

		if !authResult.Authorized {
			err := voicebackend.NewBackendError("CreateSession", voicebackend.ErrCodeAuthenticationFailed,
	...
	if b.config.AuthHook != nil {
		...
		if err != nil {
			voicebackend.RecordSpanError(span, err)
			return nil, voicebackend.NewBackendError("CreateSession", voicebackend.ErrCodeAuthorizationFailed, err)
		}

		if !authorized {
			err := voicebackend.NewBackendError("CreateSession", voicebackend.ErrCodeAuthorizationFailed,
	...
	if !allowed {
		...
		if err := rateLimiter.Wait(ctx, config.UserID); err != nil {
			voicebackend.RecordSpanError(span, err)
			return nil, voicebackend.NewBackendError("CreateSession", voicebackend.ErrCodeRateLimitExceeded, err)
		}
	}
	...
	if b.config.TelephonyHook != nil && config.Transport == "telephony" {
		...
		if phoneNumber != "" {
			...
			if err != nil {
				voicebackend.LogWithOTELContext(ctx, slog.LevelWarn, "Telephony hook routing failed, using default",
				...
				voicebackend.LogWithOTELContext(ctx, slog.LevelInfo, "Telephony hook suggests different provider",
	...
	if err != nil {
		voicebackend.RecordSpanError(span, err)
		voicebackend.LogWithOTELContext(ctx, slog.LevelError, "Failed to create LiveKit room", "error", err, "room_name", roomName)
		return nil, voicebackend.WrapError("CreateSession", err)
	}
	...
	voicebackend.LogWithOTELContext(ctx, slog.LevelInfo, "LiveKit session created successfully",
...
func (b *LiveKitBackend) CloseSession(ctx context.Context, sessionID string) error {
	...
	if !ok {
		return voicebackend.NewBackendError("CloseSession", voicebackend.ErrCodeSessionNotFound,
...
func (b *LiveKitBackend) reconnect(ctx context.Context) {
	...
	if err != nil {
		b.connectionState = iface.ConnectionStateError
		voicebackend.LogWithOTELContext(ctx, slog.LevelError, "Failed to reconnect to LiveKit", "error", err)
	} else {
		voicebackend.LogWithOTELContext(ctx, slog.LevelInfo, "Successfully reconnected to LiveKit")
	}
}
