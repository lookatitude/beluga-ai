...
func (s *MockSession) Start(ctx context.Context) error {
	...
	if s.active {
		return voicebackend.NewBackendError("Start", "session_already_active",
			nil)
	}
...
func (s *MockSession) ProcessAudio(ctx context.Context, audio []byte) error {
	...
	if !active {
		return voicebackend.NewBackendError("ProcessAudio", "session_not_active", nil)
	}
	...
	case <-time.After(5 * time.Second):
		return voicebackend.NewBackendError("ProcessAudio", voicebackend.ErrCodeTimeout,
			nil)
	}
