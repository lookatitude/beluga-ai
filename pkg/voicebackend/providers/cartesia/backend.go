...
// NewCartesiaBackend creates a new Cartesia voicebackend.
...
func (b *CartesiaBackend) Start(ctx context.Context) error {
	ctx, span := voicebackend.StartSpan(ctx, "CartesiaBackend.Start", "cartesia")
	defer span.End()
	...
	if err != nil {
		...
		voicebackend.LogWithOTELContext(ctx, slog.LevelError, "Failed to connect to Cartesia API", "error", err)
	...
	voicebackend.LogWithOTELContext(ctx, slog.LevelInfo, "Cartesia backend started successfully")
	return nil
}

// Stop stops the Cartesia backend gracefully.
func (b *CartesiaBackend) Stop(ctx context.Context) error {
	ctx, span := voicebackend.StartSpan(ctx, "CartesiaBackend.Stop", "cartesia")
	defer span.End()
	...
	voicebackend.LogWithOTELContext(ctx, slog.LevelInfo, "Cartesia backend stopped successfully")
	return nil
}
...
func (b *CartesiaBackend) CreateSession(ctx context.Context, config *iface.SessionConfig) (iface.VoiceSession, error) {
	startTime := time.Now()
	ctx, span := voicebackend.StartSpan(ctx, "CartesiaBackend.CreateSession", "cartesia")
	defer span.End()
	...
	if err != nil {
		voicebackend.RecordSpanError(span, err)
		voicebackend.LogWithOTELContext(ctx, slog.LevelError, "Failed to create Cartesia session", "error", err)
	...
	voicebackend.LogWithOTELContext(ctx, slog.LevelInfo, "Cartesia session created successfully",
