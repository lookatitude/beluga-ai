...
func (sm *SessionManager) CreateSession(ctx context.Context, sessionConfig *vbiface.SessionConfig) (vbiface.VoiceSession, error) {
	...
	if sm.config.MaxConcurrentSessions > 0 {
		if len(sm.sessions) >= sm.config.MaxConcurrentSessions {
			return nil, voicebackend.NewBackendError("CreateSession", voicebackend.ErrCodeSessionLimitExceeded,
				fmt.Errorf("maximum concurrent sessions (%d) exceeded", sm.config.MaxConcurrentSessions))
		}
	}

	// Validate session config
	if err := voicebackend.ValidateSessionConfig(sessionConfig); err != nil {
		return nil, voicebackend.NewBackendError("CreateSession", voicebackend.ErrCodeInvalidConfig, err)
	}
...
func (sm *SessionManager) GetSession(sessionID string) (vbiface.VoiceSession, error) {
	...
	if !exists {
		return nil, voicebackend.NewBackendError("GetSession", voicebackend.ErrCodeSessionNotFound,
...
func (sm *SessionManager) CloseSession(ctx context.Context, sessionID string) error {
	...
	if err := session.Stop(ctx); err != nil {
		return voicebackend.WrapError("CloseSession", err)
	}
...
func (sm *SessionManager) AddSession(sessionID string, session vbiface.VoiceSession) error {
	...
	if sm.config.MaxConcurrentSessions > 0 {
		if len(sm.sessions) >= sm.config.MaxConcurrentSessions {
			return voicebackend.NewBackendError("AddSession", voicebackend.ErrCodeSessionLimitExceeded,
				fmt.Errorf("maximum concurrent sessions (%d) exceeded", sm.config.MaxConcurrentSessions))
		}
	}
...
func (sm *SessionManager) PersistActiveSessions(ctx context.Context) error {
	...
	for _, session := range sessions {
		if err := PersistSession(ctx, store, session); err != nil {
			voicebackend.LogWithOTELContext(ctx, slog.LevelError, "Failed to persist session",
...
func (sm *SessionManager) RecoverActiveSessions(ctx context.Context) error {
	...
	if err != nil {
		return voicebackend.WrapError("RecoverActiveSessions", err)
	}
	...
	for _, sessionID := range sessionIDs {
		...
		if err != nil {
			voicebackend.LogWithOTELContext(ctx, slog.LevelWarn, "Failed to load session state",
	...
	if sm.store != nil {
		for _, sessionID := range sessionsToRemove {
			if err := sm.store.DeleteSession(ctx, sessionID); err != nil {
				voicebackend.LogWithOTELContext(ctx, slog.LevelWarn, "Failed to delete session from persistence store",
					"session_id", sessionID, "error", err)
			}
		}
	}
