...
func (po *PipelineOrchestrator) ProcessAudio(ctx context.Context, audio []byte, agentCallback func(context.Context, string) (string, error), agentInstance agentsiface.Agent) ([]byte, error) {
	ctx, span := voicebackend.StartSpan(ctx, "PipelineOrchestrator.ProcessAudio", po.config.Provider)
	defer span.End()
	...
	default:
		err := voicebackend.NewBackendError("ProcessAudio", voicebackend.ErrCodePipelineError,
...
func (po *PipelineOrchestrator) processSTTTTSPipeline(ctx context.Context, audio []byte, agentCallback func(context.Context, string) (string, error), agentInstance agentsiface.Agent) ([]byte, error) {
	ctx, span := voicebackend.StartSpan(ctx, "PipelineOrchestrator.processSTTTTSPipeline", po.config.Provider)
	defer span.End()
	startTime := time.Now()
	...
	if err != nil {
		return nil, voicebackend.WrapError("processSTTTTSPipeline", err)
	}
	...
	if sttProvider == nil {
		return nil, voicebackend.NewBackendError("processSTTTTSPipeline", voicebackend.ErrCodePipelineError,
	...
	if turnDetector != nil {
		...
		if err != nil {
			...
			voicebackend.LogWithOTELContext(ctx, slog.LevelWarn, "Turn detection failed, using fallback",
				"error", err)
	...
	case err = <-agentErrorChan:
		...
		voicebackend.LogWithOTELContext(ctx, slog.LevelError, "Agent error during response generation",
			"error", err, "transcript", transcript)
		return nil, voicebackend.NewBackendError("processSTTTTSPipeline", voicebackend.ErrCodeAgentError, err)
	case <-agentCtx.Done():
		// Timeout - return timeout error code (T291)
		err := agentCtx.Err()
		voicebackend.LogWithOTELContext(ctx, slog.LevelWarn, "Agent response timeout",
			"timeout", agentTimeout, "transcript", transcript)
		return nil, voicebackend.NewBackendError("processSTTTTSPipeline", voicebackend.ErrCodeTimeout, err)
	case <-ctx.Done():
		// Context canceled
		return nil, voicebackend.NewBackendError("processSTTTTSPipeline", voicebackend.ErrCodeContextCanceled, ctx.Err())
	}
	...
	if latency > po.config.LatencyTarget {
		voicebackend.LogWithOTELContext(ctx, slog.LevelWarn, "Latency target exceeded",
			"latency_ms", latency.Milliseconds(),
...
func (po *PipelineOrchestrator) processS2SPipeline(ctx context.Context, audio []byte, agentCallback func(context.Context, string) (string, error), agentInstance agentsiface.Agent) ([]byte, error) {
	ctx, span := voicebackend.StartSpan(ctx, "PipelineOrchestrator.processS2SPipeline", po.config.Provider)
	defer span.End()
	...
	if err != nil {
		return nil, voicebackend.WrapError("processS2SPipeline", err)
	}
	...
	if agentInstance != nil || agentCallback != nil {
		...
		if sttProvider != nil {
			...
			transcript, err := sttProvider.Transcribe(ctx, audio)
			if err != nil {
				voicebackend.LogWithOTELContext(ctx, slog.LevelWarn, "Failed to transcribe for agent, proceeding with direct S2S", "error", err)
			} else {
				// Get agent response
				if agentInstance != nil {
					// TODO: Implement proper agent execution using executor pattern
					...
					if agentCallback != nil {
						...
						if err != nil {
							voicebackend.LogWithOTELContext(ctx, slog.LevelWarn, "Agent callback failed, proceeding with direct S2S", "error", err)
						} else {
	...
	if s2sProvider == nil {
		err := voicebackend.NewBackendError("processS2SPipeline", voicebackend.ErrCodePipelineError,
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		if attempt > 0 {
			...
			select {
			case <-s2sCtx.Done():
				return nil, voicebackend.NewBackendError("processS2SPipeline", voicebackend.ErrCodeTimeout, s2sCtx.Err())
			case <-time.After(retryDelay):
			}
			voicebackend.LogWithOTELContext(ctx, slog.LevelWarn, "Retrying S2S processing due to network latency",
		...
		if !voicebackend.IsRetryableError(s2sErr) || attempt >= maxRetries {
			voicebackend.RecordSpanError(span, s2sErr)
			return nil, voicebackend.WrapError("processS2SPipeline", s2sErr)
		}
	}
	...
	if len(outputAudioData) > maxOutputSize || len(outputAudioData) < minOutputSize {
		voicebackend.LogWithOTELContext(ctx, slog.LevelWarn, "S2S output size suspicious, may be malformed",
	...
	if totalLatency > s2sTargetLatency {
		voicebackend.LogWithOTELContext(ctx, slog.LevelWarn, "S2S latency target exceeded",
			"latency_ms", totalLatency.Milliseconds(),
			"target_ms", s2sTargetLatency.Milliseconds(),
			"s2s_provider_latency_ms", s2sLatency.Milliseconds())
	} else {
		voicebackend.LogWithOTELContext(ctx, slog.LevelInfo, "S2S pipeline completed within latency target",
			"latency_ms", totalLatency.Milliseconds())
	}
...
func (po *PipelineOrchestrator) applyCustomProcessors(ctx context.Context, audio []byte, stage string) ([]byte, error) {
	...
	for _, processor := range sortedProcessors {
		...
		if err != nil {
			return nil, voicebackend.WrapError("applyCustomProcessors", err)
		}
		result = processed
	}
