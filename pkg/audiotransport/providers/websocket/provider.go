...
import (
	...
	transportiface "github.com/lookatitude/beluga-ai/pkg/audiotransport/iface"
)
...
func NewWebSocketTransport(config *audiotransport.Config) (transportiface.Transport, error) {
	if config == nil {
		return nil, audiotransport.NewTransportError("NewWebSocketTransport", audiotransport.ErrCodeInvalidConfig,
...
func (t *WebSocketTransport) SendAudio(ctx context.Context, audio []byte) error {
	...
	if !connected {
		return audiotransport.NewTransportError("SendAudio", audiotransport.ErrCodeNotConnected,
	...
	if len(audio) == 0 {
		return audiotransport.NewTransportError("SendAudio", audiotransport.ErrCodeInvalidInput,
