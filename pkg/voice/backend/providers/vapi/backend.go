...
// NewVapiBackend creates a new Vapi backend.
...
func (b *VapiBackend) Start(ctx context.Context) error {
	ctx, span := backend.StartSpan(ctx, "VapiBackend.Start", "vapi")
	defer span.End()
	...
	if err != nil {
		...
		backend.LogWithOTELContext(ctx, slog.LevelError, "Failed to connect to Vapi API", "error", err)
	...
	backend.LogWithOTELContext(ctx, slog.LevelInfo, "Vapi backend started successfully")
	return nil
}

// Stop stops the Vapi backend gracefully.
func (b *VapiBackend) Stop(ctx context.Context) error {
	ctx, span := backend.StartSpan(ctx, "VapiBackend.Stop", "vapi")
	defer span.End()
	...
	backend.LogWithOTELContext(ctx, slog.LevelInfo, "Vapi backend stopped successfully")
	return nil
}
...
func (b *VapiBackend) CreateSession(ctx context.Context, config *iface.SessionConfig) (iface.VoiceSession, error) {
	startTime := time.Now()
	ctx, span := backend.StartSpan(ctx, "VapiBackend.CreateSession", "vapi")
	defer span.End()
	...
	backend.LogWithOTELContext(ctx, slog.LevelInfo, "Vapi session created successfully",
