...
// Global registry for STT providers.
var (
...
type Registry struct {
	providers map[string]func(*Config) (iface.STTProvider, error)
	mu        sync.RWMutex
}
...
func GetRegistry() *Registry {
	registryOnce.Do(func() {
		globalRegistry = &Registry{
			providers: make(map[string]func(*Config) (iface.STTProvider, error)),
		}
	})
...
func (r *Registry) GetProvider(name string, config *Config) (iface.STTProvider, error) {
	...
	if !exists {
		return nil, NewSTTError("GetProvider", ErrCodeUnsupportedProvider,
