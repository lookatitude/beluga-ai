...
import (
	...
	"github.com/lookatitude/beluga-ai/pkg/noisereduction/iface"
)
...
func NewRNNoiseProvider(config *noisereduction.Config) (iface.NoiseCancellation, error) {
	if config == nil {
		return nil, noisereduction.NewNoiseCancellationError("NewRNNoiseProvider", noisereduction.ErrCodeInvalidConfig,
	...
	if frameSize != 480 {
		return nil, noisereduction.NewNoiseCancellationError("NewRNNoiseProvider", noisereduction.ErrCodeFrameSizeError,
			fmt.Errorf("RNNoise requires frame size of 480 samples, got %d", config.FrameSize))
	}
	if sampleRate != 48000 {
		return nil, noisereduction.NewNoiseCancellationError("NewRNNoiseProvider", noisereduction.ErrCodeSampleRateError,
	...
	if err := model.Load(); err != nil {
		return nil, noisereduction.NewNoiseCancellationError("NewRNNoiseProvider", noisereduction.ErrCodeModelLoadFailed, err)
	}
...
func (p *RNNoiseProvider) Process(ctx context.Context, audio []byte) ([]byte, error) {
	...
	if err != nil {
		return nil, noisereduction.NewNoiseCancellationError("Process", noisereduction.ErrCodeProcessingError, err)
	}
