...
// Global registry for Noise Cancellation providers.
var (
...
type Registry struct {
	providers map[string]func(*Config) (iface.NoiseCancellation, error)
	mu        sync.RWMutex
}
...
func GetRegistry() *Registry {
	registryOnce.Do(func() {
		globalRegistry = &Registry{
			providers: make(map[string]func(*Config) (iface.NoiseCancellation, error)),
		}
	})
...
func (r *Registry) GetProvider(name string, config *Config) (iface.NoiseCancellation, error) {
	...
	if !exists {
		return nil, NewNoiseCancellationError("GetProvider", ErrCodeUnsupportedProvider,
