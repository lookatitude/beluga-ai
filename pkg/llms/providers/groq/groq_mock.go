...
func (m *AdvancedMockGroqProvider) Generate(ctx context.Context, messages []schema.Message, options ...core.Option) (schema.Message, error) {
	m.mu.Lock()
	m.callCount++
...
func (m *AdvancedMockGroqProvider) StreamChat(ctx context.Context, messages []schema.Message, options ...core.Option) (<-chan iface.AIMessageChunk, error) {
	m.mu.Lock()
	m.callCount++
	shouldError := m.shouldError
	errorToReturn := m.errorToReturn
...
func (m *AdvancedMockGroqProvider) BindTools(toolsToBind []tools.Tool) iface.ChatModel {
	newMock := *m
...
func (m *AdvancedMockGroqProvider) GetModelName() string {
	return m.modelName
}

// GetProviderName implements the LLM interface.
func (m *AdvancedMockGroqProvider) GetProviderName() string {
	return ProviderName
}

// Invoke implements the Runnable interface.
func (m *AdvancedMockGroqProvider) Invoke(ctx context.Context, input any, options ...core.Option) (any, error) {
...
func (m *AdvancedMockGroqProvider) Batch(ctx context.Context, inputs []any, options ...core.Option) ([]any, error) {
...
func (m *AdvancedMockGroqProvider) Stream(ctx context.Context, input any, options ...core.Option) (<-chan any, error) {
	messages, err := llms.EnsureMessages(input)
	if err != nil {
		return nil, err
	}
...
func (m *AdvancedMockGroqProvider) CheckHealth() map[string]any {
	m.mu.RLock()
	defer m.mu.RUnlock()
...
func (m *AdvancedMockGroqProvider) SetError(shouldError bool, err error) {
	m.mu.Lock()
	defer m.mu.Unlock()
...
func (m *AdvancedMockGroqProvider) SetDelay(delay time.Duration) {
	m.mu.Lock()
	defer m.mu.Unlock()
...
func (m *AdvancedMockGroqProvider) SetRateLimit(enabled bool) {
	m.mu.Lock()
	defer m.mu.Unlock()
...
func (m *AdvancedMockGroqProvider) GetCallCount() int {
	m.mu.RLock()
	defer m.mu.RUnlock()
	return m.callCount
}
...
func (m *AdvancedMockGroqProvider) Reset() {
	m.mu.Lock()
	defer m.mu.Unlock()
	m.callCount = 0
	m.shouldError = false
...
// Ensure AdvancedMockGroqProvider implements the interfaces.
var (
	_ iface.ChatModel = (*AdvancedMockGroqProvider)(nil)
)
