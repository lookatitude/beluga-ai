...
// AdvancedMockGrokProvider provides a comprehensive mock implementation for testing Grok provider.
type AdvancedMockGrokProvider struct {
	mu                *sync.RWMutex
	callCount         int
	shouldError       bool
	errorToReturn     error
...
// NewAdvancedMockGrokProvider creates a new advanced mock with configurable behavior.
func NewAdvancedMockGrokProvider(modelName string) *AdvancedMockGrokProvider {
	mock := &AdvancedMockGrokProvider{
		mu:        &sync.RWMutex{},
		modelName: modelName,
		responses: []string{
...
func (m *AdvancedMockGrokProvider) Generate(ctx context.Context, messages []schema.Message, options ...core.Option) (schema.Message, error) {
	m.mu.Lock()
	m.callCount++
...
func (m *AdvancedMockGrokProvider) StreamChat(ctx context.Context, messages []schema.Message, options ...core.Option) (<-chan iface.AIMessageChunk, error) {
	m.mu.Lock()
	m.callCount++
	shouldError := m.shouldError
	errorToReturn := m.errorToReturn
...
func (m *AdvancedMockGrokProvider) BindTools(toolsToBind []tools.Tool) iface.ChatModel {
	newMock := *m
...
func (m *AdvancedMockGrokProvider) GetModelName() string {
	return m.modelName
}

// GetProviderName implements the LLM interface.
func (m *AdvancedMockGrokProvider) GetProviderName() string {
	return ProviderName
}

// Invoke implements the Runnable interface.
func (m *AdvancedMockGrokProvider) Invoke(ctx context.Context, input any, options ...core.Option) (any, error) {
...
func (m *AdvancedMockGrokProvider) Batch(ctx context.Context, inputs []any, options ...core.Option) ([]any, error) {
...
func (m *AdvancedMockGrokProvider) Stream(ctx context.Context, input any, options ...core.Option) (<-chan any, error) {
	messages, err := llms.EnsureMessages(input)
	if err != nil {
		return nil, err
	}
...
func (m *AdvancedMockGrokProvider) CheckHealth() map[string]any {
	m.mu.RLock()
	defer m.mu.RUnlock()
...
func (m *AdvancedMockGrokProvider) SetError(shouldError bool, err error) {
	m.mu.Lock()
	defer m.mu.Unlock()
...
func (m *AdvancedMockGrokProvider) SetDelay(delay time.Duration) {
	m.mu.Lock()
	defer m.mu.Unlock()
...
func (m *AdvancedMockGrokProvider) SetRateLimit(enabled bool) {
	m.mu.Lock()
	defer m.mu.Unlock()
...
func (m *AdvancedMockGrokProvider) GetCallCount() int {
	m.mu.RLock()
	defer m.mu.RUnlock()
	return m.callCount
}
...
func (m *AdvancedMockGrokProvider) Reset() {
	m.mu.Lock()
	defer m.mu.Unlock()
	m.callCount = 0
	m.shouldError = false
...
// Ensure AdvancedMockGrokProvider implements the interfaces.
var (
	_ iface.ChatModel = (*AdvancedMockGrokProvider)(nil)
)
