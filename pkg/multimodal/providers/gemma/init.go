...
func init() {
	// Register Gemma provider with the global registry
	// Use reflection to avoid import cycles
	registry.GetRegistry().Register("gemma", func(ctx context.Context, config any) (multimodaliface.MultimodalModel, error) {
		tracer := otel.Tracer("github.com/lookatitude/beluga-ai/pkg/multimodal/providers/gemma")
		ctx, span := tracer.Start(ctx, "gemma.init")
		defer span.End()
		...
		// Extract Gemma config from multimodal config
		...
		gemmaConfig := FromMultimodalConfig(multimodalConfig)

		// Create provider
		return NewGemmaProvider(gemmaConfig)
	})
}
