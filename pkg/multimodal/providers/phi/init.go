...
func init() {
	// Register Phi provider with the global registry
	// Use reflection to avoid import cycles
	registry.GetRegistry().Register("phi", func(ctx context.Context, config any) (multimodaliface.MultimodalModel, error) {
		tracer := otel.Tracer("github.com/lookatitude/beluga-ai/pkg/multimodal/providers/phi")
		ctx, span := tracer.Start(ctx, "phi.init")
		defer span.End()
		...
		// Extract Phi config from multimodal config
		...
		phiConfig := FromMultimodalConfig(multimodalConfig)

		// Create provider
		return NewPhiProvider(phiConfig)
	})
}
