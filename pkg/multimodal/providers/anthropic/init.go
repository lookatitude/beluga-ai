...
func init() {
	...
	registry.GetRegistry().Register("anthropic", func(ctx context.Context, config any) (multimodaliface.MultimodalModel, error) {
		tracer := otel.Tracer("github.com/lookatitude/beluga-ai/pkg/multimodal/providers/anthropic")
		ctx, span := tracer.Start(ctx, "anthropic.init")
		defer span.End()
		...
		// Extract Anthropic config from multimodal config
		...
		// Convert to Anthropic config
		anthropicConfig := FromMultimodalConfig(multimodalConfig)
