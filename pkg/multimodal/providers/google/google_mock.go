...
func (m *AdvancedMockGoogleProvider) Process(ctx context.Context, input *types.MultimodalInput) (*types.MultimodalOutput, error) {
	m.mu.Lock()
	m.callCount++
	...
	output := &types.MultimodalOutput{
		ID:            "mock-google-output-" + time.Now().Format(time.RFC3339Nano),
		InputID:       input.ID,
		ContentBlocks: input.ContentBlocks,
		Metadata:      map[string]any{"provider": "google", "model": m.modelName},
		Confidence:    0.95,
		Provider:      "google",
		Model:         m.modelName,
		CreatedAt:     time.Now(),
...
func (m *AdvancedMockGoogleProvider) ProcessStream(ctx context.Context, input *types.MultimodalInput) (<-chan *types.MultimodalOutput, error) {
	m.mu.Lock()
	m.callCount++
	shouldError := m.shouldError
	errorToReturn := m.errorToReturn
	...
	go func() {
		...
		output := &types.MultimodalOutput{
			ID:            "mock-google-stream-" + time.Now().Format(time.RFC3339Nano),
			InputID:       input.ID,
			ContentBlocks: input.ContentBlocks,
			Metadata:      map[string]any{"provider": "google", "model": m.modelName, "streaming": true},
			Confidence:    0.95,
			Provider:      "google",
			Model:         m.modelName,
			CreatedAt:     time.Now(),
...
func (m *AdvancedMockGoogleProvider) GetCapabilities(ctx context.Context) (*types.ModalityCapabilities, error) {
	m.mu.RLock()
	defer m.mu.RUnlock()
...
func (m *AdvancedMockGoogleProvider) SupportsModality(ctx context.Context, modality string) (bool, error) {
	m.mu.RLock()
	defer m.mu.RUnlock()
...
func (m *AdvancedMockGoogleProvider) CheckHealth(ctx context.Context) error {
	m.mu.RLock()
	defer m.mu.RUnlock()
...
func (m *AdvancedMockGoogleProvider) WithMockError(shouldError bool, err error) *AdvancedMockGoogleProvider {
	m.mu.Lock()
	defer m.mu.Unlock()
...
func (m *AdvancedMockGoogleProvider) WithMockDelay(delay time.Duration) *AdvancedMockGoogleProvider {
	m.mu.Lock()
	defer m.mu.Unlock()
...
func (m *AdvancedMockGoogleProvider) WithRateLimit(enabled bool) *AdvancedMockGoogleProvider {
	m.mu.Lock()
	defer m.mu.Unlock()
...
func (m *AdvancedMockGoogleProvider) GetLastInput() *types.MultimodalInput {
	m.mu.RLock()
	defer m.mu.RUnlock()
	return m.lastInput
}
...
func (m *AdvancedMockGoogleProvider) GetLastOutput() *types.MultimodalOutput {
	m.mu.RLock()
	defer m.mu.RUnlock()
	return m.lastOutput
}
