...
func init() {
	// Register Google Vertex AI provider with the global registry
	// Use reflection to avoid import cycles
	registry.GetRegistry().Register("google", func(ctx context.Context, config any) (multimodaliface.MultimodalModel, error) {
		tracer := otel.Tracer("github.com/lookatitude/beluga-ai/pkg/multimodal/providers/google")
		ctx, span := tracer.Start(ctx, "google.init")
		defer span.End()
		...
		// Extract Google config from multimodal config
		...
		// Try to get ProviderSpecific field for project_id and location
		...
		if providerSpecificField.IsValid() && providerSpecificField.Kind() == reflect.Map {
			multimodalConfig.ProviderSpecific = providerSpecificField.Interface().(map[string]any)
		}

		// Convert to Google config
		googleConfig := FromMultimodalConfig(multimodalConfig)
