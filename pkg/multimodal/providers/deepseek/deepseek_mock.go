...
// AdvancedMockDeepSeekProvider provides a comprehensive mock implementation for testing DeepSeek multimodal provider.
type AdvancedMockDeepSeekProvider struct {
	mock.Mock
	mu                sync.RWMutex
	callCount         int
	shouldError       bool
...
// NewAdvancedMockDeepSeekProvider creates a new advanced mock with configurable behavior.
func NewAdvancedMockDeepSeekProvider(modelName string) *AdvancedMockDeepSeekProvider {
	mock := &AdvancedMockDeepSeekProvider{
		modelName: modelName,
		capabilities: &types.ModalityCapabilities{
			Text:                  true,
			Image:                 true,
...
func (m *AdvancedMockDeepSeekProvider) Process(ctx context.Context, input *types.MultimodalInput) (*types.MultimodalOutput, error) {
	m.mu.Lock()
	m.callCount++
	...
	output := &types.MultimodalOutput{
		ID:            "mock-deepseek-output-" + time.Now().Format(time.RFC3339Nano),
		InputID:       input.ID,
		ContentBlocks: input.ContentBlocks,
		Metadata:      map[string]any{"provider": "deepseek", "model": m.modelName},
		Confidence:    0.95,
		Provider:      "deepseek",
		Model:         m.modelName,
		CreatedAt:     time.Now(),
...
func (m *AdvancedMockDeepSeekProvider) ProcessStream(ctx context.Context, input *types.MultimodalInput) (<-chan *types.MultimodalOutput, error) {
	m.mu.Lock()
	m.callCount++
	shouldError := m.shouldError
	errorToReturn := m.errorToReturn
	...
	go func() {
		...
		output := &types.MultimodalOutput{
			ID:            "mock-deepseek-stream-" + time.Now().Format(time.RFC3339Nano),
			InputID:       input.ID,
			ContentBlocks: input.ContentBlocks,
			Metadata:      map[string]any{"provider": "deepseek", "model": m.modelName, "streaming": true},
			Confidence:    0.95,
			Provider:      "deepseek",
			Model:         m.modelName,
			CreatedAt:     time.Now(),
...
func (m *AdvancedMockDeepSeekProvider) GetCapabilities(ctx context.Context) (*types.ModalityCapabilities, error) {
	m.mu.RLock()
	defer m.mu.RUnlock()
...
func (m *AdvancedMockDeepSeekProvider) SupportsModality(ctx context.Context, modality string) (bool, error) {
	m.mu.RLock()
	defer m.mu.RUnlock()
...
func (m *AdvancedMockDeepSeekProvider) CheckHealth(ctx context.Context) error {
	m.mu.RLock()
	defer m.mu.RUnlock()
...
func (m *AdvancedMockDeepSeekProvider) WithMockError(shouldError bool, err error) *AdvancedMockDeepSeekProvider {
	m.mu.Lock()
	defer m.mu.Unlock()
...
func (m *AdvancedMockDeepSeekProvider) WithMockDelay(delay time.Duration) *AdvancedMockDeepSeekProvider {
	m.mu.Lock()
	defer m.mu.Unlock()
...
func (m *AdvancedMockDeepSeekProvider) WithRateLimit(enabled bool) *AdvancedMockDeepSeekProvider {
	m.mu.Lock()
	defer m.mu.Unlock()
...
func (m *AdvancedMockDeepSeekProvider) GetCallCount() int {
	m.mu.RLock()
	defer m.mu.RUnlock()
	return m.callCount
}
...
func (m *AdvancedMockDeepSeekProvider) GetLastInput() *types.MultimodalInput {
	m.mu.RLock()
	defer m.mu.RUnlock()
	return m.lastInput
}
...
func (m *AdvancedMockDeepSeekProvider) GetLastOutput() *types.MultimodalOutput {
	m.mu.RLock()
	defer m.mu.RUnlock()
	return m.lastOutput
}

// Ensure AdvancedMockDeepSeekProvider implements iface.MultimodalModel
var _ iface.MultimodalModel = (*AdvancedMockDeepSeekProvider)(nil)
