...
// AdvancedMockGeminiProvider provides a comprehensive mock implementation for testing Gemini multimodal provider.
...
func (m *AdvancedMockGeminiProvider) Process(ctx context.Context, input *types.MultimodalInput) (*types.MultimodalOutput, error) {
	m.mu.Lock()
	m.callCount++
	...
	output := &types.MultimodalOutput{
		ID:            "mock-gemini-output-" + time.Now().Format(time.RFC3339Nano),
		InputID:       input.ID,
		ContentBlocks: input.ContentBlocks,
		Metadata:      map[string]any{"provider": "gemini", "model": m.modelName},
		Confidence:    0.95,
		Provider:      "gemini",
		Model:         m.modelName,
		CreatedAt:     time.Now(),
...
func (m *AdvancedMockGeminiProvider) ProcessStream(ctx context.Context, input *types.MultimodalInput) (<-chan *types.MultimodalOutput, error) {
	m.mu.Lock()
	m.callCount++
	shouldError := m.shouldError
	errorToReturn := m.errorToReturn
	...
	go func() {
		...
		output := &types.MultimodalOutput{
			ID:            "mock-gemini-stream-" + time.Now().Format(time.RFC3339Nano),
			InputID:       input.ID,
			ContentBlocks: input.ContentBlocks,
			Metadata:      map[string]any{"provider": "gemini", "model": m.modelName, "streaming": true},
			Confidence:    0.95,
			Provider:      "gemini",
			Model:         m.modelName,
			CreatedAt:     time.Now(),
...
func (m *AdvancedMockGeminiProvider) GetCapabilities(ctx context.Context) (*types.ModalityCapabilities, error) {
	m.mu.RLock()
	defer m.mu.RUnlock()
...
func (m *AdvancedMockGeminiProvider) SupportsModality(ctx context.Context, modality string) (bool, error) {
	m.mu.RLock()
	defer m.mu.RUnlock()
...
func (m *AdvancedMockGeminiProvider) CheckHealth(ctx context.Context) error {
	m.mu.RLock()
	defer m.mu.RUnlock()
...
func (m *AdvancedMockGeminiProvider) WithMockError(shouldError bool, err error) *AdvancedMockGeminiProvider {
	m.mu.Lock()
	defer m.mu.Unlock()
...
func (m *AdvancedMockGeminiProvider) WithMockDelay(delay time.Duration) *AdvancedMockGeminiProvider {
	m.mu.Lock()
	defer m.mu.Unlock()
...
func (m *AdvancedMockGeminiProvider) WithRateLimit(enabled bool) *AdvancedMockGeminiProvider {
	m.mu.Lock()
	defer m.mu.Unlock()
...
func (m *AdvancedMockGeminiProvider) GetLastInput() *types.MultimodalInput {
	m.mu.RLock()
	defer m.mu.RUnlock()
	return m.lastInput
}
...
func (m *AdvancedMockGeminiProvider) GetLastOutput() *types.MultimodalOutput {
	m.mu.RLock()
	defer m.mu.RUnlock()
	return m.lastOutput
}

// Ensure AdvancedMockGeminiProvider implements iface.MultimodalModel
var _ iface.MultimodalModel = (*AdvancedMockGeminiProvider)(nil)
