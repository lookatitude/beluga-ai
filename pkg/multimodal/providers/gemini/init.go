...
func init() {
	...
	registry.GetRegistry().Register("gemini", func(ctx context.Context, config any) (multimodaliface.MultimodalModel, error) {
		tracer := otel.Tracer("github.com/lookatitude/beluga-ai/pkg/multimodal/providers/gemini")
		ctx, span := tracer.Start(ctx, "gemini.init")
		defer span.End()
		...
		// Extract Gemini config from multimodal config
		...
		geminiConfig := FromMultimodalConfig(multimodalConfig)
