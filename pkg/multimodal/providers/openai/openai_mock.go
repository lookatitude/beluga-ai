...
func (m *AdvancedMockOpenAIProvider) Process(ctx context.Context, input *types.MultimodalInput) (*types.MultimodalOutput, error) {
	m.mu.Lock()
	m.callCount++
	...
	output := &types.MultimodalOutput{
		ID:            "mock-openai-output-" + time.Now().Format(time.RFC3339Nano),
		InputID:       input.ID,
		ContentBlocks: input.ContentBlocks,
		Metadata:      map[string]any{"provider": "openai", "model": m.modelName},
		Confidence:    0.95,
		Provider:      "openai",
		Model:         m.modelName,
		CreatedAt:     time.Now(),
...
func (m *AdvancedMockOpenAIProvider) ProcessStream(ctx context.Context, input *types.MultimodalInput) (<-chan *types.MultimodalOutput, error) {
	m.mu.Lock()
	m.callCount++
	shouldError := m.shouldError
	errorToReturn := m.errorToReturn
	...
	go func() {
		...
		output := &types.MultimodalOutput{
			ID:            "mock-openai-stream-" + time.Now().Format(time.RFC3339Nano),
			InputID:       input.ID,
			ContentBlocks: input.ContentBlocks,
			Metadata:      map[string]any{"provider": "openai", "model": m.modelName, "streaming": true},
			Confidence:    0.95,
			Provider:      "openai",
			Model:         m.modelName,
			CreatedAt:     time.Now(),
...
func (m *AdvancedMockOpenAIProvider) GetCapabilities(ctx context.Context) (*types.ModalityCapabilities, error) {
	m.mu.RLock()
	defer m.mu.RUnlock()
...
func (m *AdvancedMockOpenAIProvider) SupportsModality(ctx context.Context, modality string) (bool, error) {
	m.mu.RLock()
	defer m.mu.RUnlock()
...
func (m *AdvancedMockOpenAIProvider) CheckHealth(ctx context.Context) error {
	m.mu.RLock()
	defer m.mu.RUnlock()
...
func (m *AdvancedMockOpenAIProvider) WithMockError(shouldError bool, err error) *AdvancedMockOpenAIProvider {
	m.mu.Lock()
	defer m.mu.Unlock()
...
func (m *AdvancedMockOpenAIProvider) WithMockDelay(delay time.Duration) *AdvancedMockOpenAIProvider {
	m.mu.Lock()
	defer m.mu.Unlock()
...
func (m *AdvancedMockOpenAIProvider) WithRateLimit(enabled bool) *AdvancedMockOpenAIProvider {
	m.mu.Lock()
	defer m.mu.Unlock()
...
func (m *AdvancedMockOpenAIProvider) GetLastInput() *types.MultimodalInput {
	m.mu.RLock()
	defer m.mu.RUnlock()
	return m.lastInput
}
...
func (m *AdvancedMockOpenAIProvider) GetLastOutput() *types.MultimodalOutput {
	m.mu.RLock()
	defer m.mu.RUnlock()
	return m.lastOutput
}
