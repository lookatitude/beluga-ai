// Package iface defines the core interfaces and error types for the retrievers package.
package iface
...
// NewProviderNotFoundError creates an error for when a provider is not registered.
func NewProviderNotFoundError(op, provider string) *RetrieverError {
	return &RetrieverError{
		Op:      op,
		Err:     fmt.Errorf("provider '%s' not registered", provider),
		Code:    ErrCodeProviderNotFound,
		Message: fmt.Sprintf("retriever provider '%s' is not registered in the registry", provider),
	}
}
