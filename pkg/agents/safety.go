...
func (sc *SafetyChecker) initializePatterns() {
	...
	sc.harmfulPatterns = []*regexp.Regexp{
		regexp.MustCompile(`(?i)\b(how\s+to|tutorial|guide)\b.*?\b(make|build|create)\b.*?\b(bomb|weapon|explosive|drug)\b`),
		regexp.MustCompile(`(?i)\b(hack|crack|exploit)\b.*?\b(password|account|system)\b`),
		regexp.MustCompile(`(?i)\b(illegal|criminal|felony)\b.*?\b(activity|behavior|action)\b`),
	}
}
