...
// AdvancedMockPineconeStore provides a comprehensive mock implementation for testing Pinecone provider.
type AdvancedMockPineconeStore struct {
	errorToReturn error
	mock.Mock
	indexName         string
	name              string
...
func (m *AdvancedMockPineconeStore) AddDocuments(ctx context.Context, documents []schema.Document, opts ...vectorstores.Option) ([]string, error) {
	m.mu.Lock()
	m.callCount++
	...
	for i, doc := range documents {
		id := fmt.Sprintf("pinecone_doc_%d", nextID)
		nextID++
		ids[i] = id
...
func (m *AdvancedMockPineconeStore) DeleteDocuments(ctx context.Context, ids []string, opts ...vectorstores.Option) error {
	m.mu.Lock()
	m.callCount++
	shouldError := m.shouldError
	errorToReturn := m.errorToReturn
...
func (m *AdvancedMockPineconeStore) SimilaritySearch(ctx context.Context, queryVector []float32, k int, opts ...vectorstores.Option) ([]schema.Document, []float32, error) {
	m.mu.Lock()
	m.callCount++
	m.mu.Unlock()
...
func (m *AdvancedMockPineconeStore) SimilaritySearchByQuery(ctx context.Context, query string, k int, embedder vectorstores.Embedder, opts ...vectorstores.Option) ([]schema.Document, []float32, error) {
	m.mu.Lock()
	m.callCount++
	m.mu.Unlock()
...
func (m *AdvancedMockPineconeStore) AsRetriever(opts ...vectorstores.Option) vectorstores.Retriever {
...
func (m *AdvancedMockPineconeStore) GetName() string {
	return m.name
}

// SetError configures the mock to return an error.
func (m *AdvancedMockPineconeStore) SetError(shouldError bool, err error) {
	m.mu.Lock()
	defer m.mu.Unlock()
...
func (m *AdvancedMockPineconeStore) SetDelay(delay time.Duration) {
	m.mu.Lock()
	defer m.mu.Unlock()
...
func (m *AdvancedMockPineconeStore) GetCallCount() int {
	m.mu.RLock()
	defer m.mu.RUnlock()
	return m.callCount
}
...
func (m *AdvancedMockPineconeStore) Reset() {
	m.mu.Lock()
	defer m.mu.Unlock()
	m.callCount = 0
	m.shouldError = false
...
type simpleMockRetriever struct {
	store *AdvancedMockPineconeStore
	k     int
}
...
var _ vectorstores.VectorStore = (*AdvancedMockPineconeStore)(nil)
