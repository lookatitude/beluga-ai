...
func (m *AdvancedMockWeaviateStore) AddDocuments(ctx context.Context, documents []schema.Document, opts ...vectorstores.Option) ([]string, error) {
	m.mu.Lock()
	m.callCount++
	...
	for i, doc := range documents {
		id := fmt.Sprintf("weaviate_doc_%d", nextID)
		nextID++
		ids[i] = id
...
func (m *AdvancedMockWeaviateStore) DeleteDocuments(ctx context.Context, ids []string, opts ...vectorstores.Option) error {
	m.mu.Lock()
	m.callCount++
	shouldError := m.shouldError
	errorToReturn := m.errorToReturn
...
func (m *AdvancedMockWeaviateStore) SimilaritySearch(ctx context.Context, queryVector []float32, k int, opts ...vectorstores.Option) ([]schema.Document, []float32, error) {
	m.mu.Lock()
	m.callCount++
	m.mu.Unlock()
...
func (m *AdvancedMockWeaviateStore) SimilaritySearchByQuery(ctx context.Context, query string, k int, embedder vectorstores.Embedder, opts ...vectorstores.Option) ([]schema.Document, []float32, error) {
	m.mu.Lock()
	m.callCount++
	m.mu.Unlock()
...
func (m *AdvancedMockWeaviateStore) AsRetriever(opts ...vectorstores.Option) vectorstores.Retriever {
...
func (m *AdvancedMockWeaviateStore) GetName() string {
	return m.name
}

// SetError configures the mock to return an error.
func (m *AdvancedMockWeaviateStore) SetError(shouldError bool, err error) {
	m.mu.Lock()
	defer m.mu.Unlock()
...
func (m *AdvancedMockWeaviateStore) SetDelay(delay time.Duration) {
	m.mu.Lock()
	defer m.mu.Unlock()
...
func (m *AdvancedMockWeaviateStore) GetCallCount() int {
	m.mu.RLock()
	defer m.mu.RUnlock()
	return m.callCount
}
...
func (m *AdvancedMockWeaviateStore) Reset() {
	m.mu.Lock()
	defer m.mu.Unlock()
	m.callCount = 0
	m.shouldError = false
...
type simpleMockRetriever struct {
	store *AdvancedMockWeaviateStore
	k     int
}
