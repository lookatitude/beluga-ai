...
func NewAdvancedMockPgVectorStore(connectionString, tableName string) *AdvancedMockPgVectorStore {
	mock := &AdvancedMockPgVectorStore{
...
func (m *AdvancedMockPgVectorStore) AddDocuments(ctx context.Context, documents []schema.Document, opts ...vectorstores.Option) ([]string, error) {
	m.mu.Lock()
	m.callCount++
	...
	for i, doc := range documents {
		id := fmt.Sprintf("pgvector_doc_%d", nextID)
		nextID++
		ids[i] = id
...
func (m *AdvancedMockPgVectorStore) DeleteDocuments(ctx context.Context, ids []string, opts ...vectorstores.Option) error {
	m.mu.Lock()
	m.callCount++
	shouldError := m.shouldError
	errorToReturn := m.errorToReturn
...
func (m *AdvancedMockPgVectorStore) SimilaritySearch(ctx context.Context, queryVector []float32, k int, opts ...vectorstores.Option) ([]schema.Document, []float32, error) {
	m.mu.Lock()
	m.callCount++
	m.mu.Unlock()
...
func (m *AdvancedMockPgVectorStore) SimilaritySearchByQuery(ctx context.Context, query string, k int, embedder vectorstores.Embedder, opts ...vectorstores.Option) ([]schema.Document, []float32, error) {
	m.mu.Lock()
	m.callCount++
	m.mu.Unlock()
...
func (m *AdvancedMockPgVectorStore) AsRetriever(opts ...vectorstores.Option) vectorstores.Retriever {
...
func (m *AdvancedMockPgVectorStore) GetName() string {
	return m.name
}

// SetError configures the mock to return an error.
func (m *AdvancedMockPgVectorStore) SetError(shouldError bool, err error) {
	m.mu.Lock()
	defer m.mu.Unlock()
...
func (m *AdvancedMockPgVectorStore) SetDelay(delay time.Duration) {
	m.mu.Lock()
	defer m.mu.Unlock()
...
func (m *AdvancedMockPgVectorStore) GetCallCount() int {
	m.mu.RLock()
	defer m.mu.RUnlock()
	return m.callCount
}
...
func (m *AdvancedMockPgVectorStore) Reset() {
	m.mu.Lock()
	defer m.mu.Unlock()
	m.callCount = 0
	m.shouldError = false
...
type simpleMockRetriever struct {
	store *AdvancedMockPgVectorStore
	k     int
}
