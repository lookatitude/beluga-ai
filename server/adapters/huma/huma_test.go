...
func TestRegistry(t *testing.T) {
	t.Run("huma is registered", func(t *testing.T) {
		names := server.List()
		found := false
		for _, n := range names {
			if n == "huma" {
		...
		if !found {
			t.Fatalf("expected 'huma' in registry, got %v", names)
		}
	})

	t.Run("New returns huma adapter", func(t *testing.T) {
...
func TestAdapter_API(t *testing.T) {
	a := New(server.Config{})
	if a.API() == nil {
...
func TestAdapter_Mux(t *testing.T) {
	a := New(server.Config{})
	if a.Mux() == nil {
...
func TestAdapter_CustomTitle(t *testing.T) {
	a := New(server.Config{
