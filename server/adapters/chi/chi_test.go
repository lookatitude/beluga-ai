...
func TestRegistry(t *testing.T) {
	t.Run("chi is registered", func(t *testing.T) {
		names := server.List()
		found := false
		for _, n := range names {
			if n == "chi" {
		...
		if !found {
			t.Fatalf("expected 'chi' in registry, got %v", names)
		}
	})

	t.Run("New returns chi adapter", func(t *testing.T) {
...
func TestAdapter_Invoke(t *testing.T) {
	...
	a.Router().(http.Handler).ServeHTTP(w, req)
...
func TestAdapter_Router(t *testing.T) {
	a := New(server.Config{})
	if a.Router() == nil {
