...
func TestRegistry(t *testing.T) {
	t.Run("echo is registered", func(t *testing.T) {
		names := server.List()
		found := false
		for _, n := range names {
			if n == "echo" {
		...
		if !found {
			t.Fatalf("expected 'echo' in registry, got %v", names)
		}
	})

	t.Run("New returns echo adapter", func(t *testing.T) {
...
func TestAdapter_Echo(t *testing.T) {
	a := New(server.Config{})
	if a.Echo() == nil {
