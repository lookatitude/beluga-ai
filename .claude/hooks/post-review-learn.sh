...
REVIEWER_RULES="$CLAUDE_DIR/agents/reviewer-security/rules"
IMPLEMENTER_RULES="$CLAUDE_DIR/agents/developer-go/rules"
...
RULEEOF
    ...
    echo "- [reviewer-security] $DATE [post-review-learn] — review $TASK_ID ($VERDICT) → $SUFFIX" >> "$LEARNINGS_INDEX"
fi
...
RULEEOF
    ...
    echo "- [developer-go] $DATE [post-review-learn] — cross-pollinated from review $TASK_ID → $SUFFIX" >> "$LEARNINGS_INDEX"
fi
