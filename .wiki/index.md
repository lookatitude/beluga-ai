...
- **C-006** — `docs/` has drifted from codebase: providers.md undercounts, first-agent.md uses non-existent APIs, README has unshipped features. See `.wiki/corrections/docs-drift.md`.
- **C-007** — `gh pr view --json state` returning `MERGED` does NOT confirm code is in `main`. Always verify with `ls <path>` in the worktree. See `.wiki/corrections/docs-drift.md`.
- **C-009** — This repo uses a multi-branch merge workflow. PRs merge into staging/integration before `main`. Canonical presence check: `git ls-files origin/main -- <path>`. See `.wiki/corrections/docs-drift.md`.
