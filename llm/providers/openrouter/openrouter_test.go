...
func mockResponse(content string) string {
	resp := map[string]any{
		"id": "chatcmpl-or", "object": "chat.completion",
		"created": 1700000000, "model": "meta-llama/llama-3.1-70b-instruct",
...
func streamResponse(deltas []string) string {
	...
	for _, d := range deltas {
		chunk := map[string]any{
			"id": "chatcmpl-or", "object": "chat.completion.chunk",
			"created": 1700000000, "model": "meta-llama/llama-3.1-70b-instruct",
			"choices": []map[string]any{{
...
func TestNew(t *testing.T) {
	m, err := New(config.ProviderConfig{Model: "meta-llama/llama-3.1-70b-instruct", APIKey: "test"})
	if err != nil {
		t.Fatalf("New() error: %v", err)
	}
	if m.ModelID() != "meta-llama/llama-3.1-70b-instruct" {
		t.Errorf("ModelID() = %q, want %q", m.ModelID(), "meta-llama/llama-3.1-70b-instruct")
	}
}
...
func TestGenerate(t *testing.T) {
	ts := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		fmt.Fprint(w, mockResponse("Hello from OpenRouter!"))
	}))
	defer ts.Close()

	m, _ := New(config.ProviderConfig{
		Model: "meta-llama/llama-3.1-70b-instruct", APIKey: "test", BaseURL: ts.URL,
	})
...
func TestStream(t *testing.T) {
	ts := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		fmt.Fprint(w, streamResponse([]string{"Open", "Router"}))
	}))
	defer ts.Close()
	...
	if text.String() != "OpenRouter" {
		t.Errorf("text = %q, want %q", text.String(), "OpenRouter")
	}
}

func TestBindTools(t *testing.T) {
	m, _ := New(config.ProviderConfig{Model: "meta-llama/llama-3.1-70b-instruct", APIKey: "test"})
	bound := m.BindTools([]schema.ToolDefinition{
		{Name: "search", Description: "search"},
	})
	if bound.ModelID() != "meta-llama/llama-3.1-70b-instruct" {
		t.Errorf("ModelID = %q", bound.ModelID())
	}
}

func TestRegistryNew(t *testing.T) {
	m, err := llm.New("openrouter", config.ProviderConfig{
		Model: "meta-llama/llama-3.1-70b-instruct", APIKey: "test",
	})
	if err != nil {
