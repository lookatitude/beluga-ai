...
func mockResponse(content string) string {
	resp := map[string]any{
		"id": "chatcmpl-sn", "object": "chat.completion",
		"created": 1700000000, "model": "Meta-Llama-3.3-70B-Instruct",
...
func streamResponse(deltas []string) string {
	...
	for _, d := range deltas {
		chunk := map[string]any{
			"id": "chatcmpl-sn", "object": "chat.completion.chunk",
			"created": 1700000000, "model": "Meta-Llama-3.3-70B-Instruct",
			"choices": []map[string]any{{
...
func TestRegistration(t *testing.T) {
	...
	if !found {
		t.Error("sambanova provider not registered")
	}
}

func TestNew(t *testing.T) {
	m, err := New(config.ProviderConfig{Model: "Meta-Llama-3.3-70B-Instruct", APIKey: "test"})
	if err != nil {
		t.Fatalf("New() error: %v", err)
	}
	if m.ModelID() != "Meta-Llama-3.3-70B-Instruct" {
		t.Errorf("ModelID() = %q, want %q", m.ModelID(), "Meta-Llama-3.3-70B-Instruct")
	}
}
...
func TestGenerate(t *testing.T) {
	ts := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		fmt.Fprint(w, mockResponse("Hello from SambaNova!"))
	}))
	defer ts.Close()

	m, _ := New(config.ProviderConfig{
		Model: "Meta-Llama-3.3-70B-Instruct", APIKey: "test", BaseURL: ts.URL,
	})
	...
	if resp.Text() != "Hello from SambaNova!" {
		t.Errorf("text = %q, want %q", resp.Text(), "Hello from SambaNova!")
	}
}
...
func TestStream(t *testing.T) {
	ts := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		fmt.Fprint(w, streamResponse([]string{"Samba", "Nova"}))
	}))
	defer ts.Close()
	...
	if text.String() != "SambaNova" {
		t.Errorf("text = %q, want %q", text.String(), "SambaNova")
	}
}

func TestBindTools(t *testing.T) {
	m, _ := New(config.ProviderConfig{Model: "Meta-Llama-3.3-70B-Instruct", APIKey: "test"})
	bound := m.BindTools([]schema.ToolDefinition{
		{Name: "search", Description: "search"},
	})
	if bound.ModelID() != "Meta-Llama-3.3-70B-Instruct" {
		t.Errorf("ModelID = %q", bound.ModelID())
	}
}

func TestRegistryNew(t *testing.T) {
	m, err := llm.New("sambanova", config.ProviderConfig{
		Model: "Meta-Llama-3.3-70B-Instruct", APIKey: "test",
	})
	if err != nil {
...
func TestDefaultBaseURL(t *testing.T) {
	if defaultBaseURL != "https://api.sambanova.ai/v1" {
