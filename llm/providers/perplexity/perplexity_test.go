...
func mockResponse(content string) string {
	resp := map[string]any{
		"id": "chatcmpl-pplx", "object": "chat.completion",
		"created": 1700000000, "model": "sonar-pro",
...
func streamResponse(deltas []string) string {
	...
	for _, d := range deltas {
		chunk := map[string]any{
			"id": "chatcmpl-pplx", "object": "chat.completion.chunk",
			"created": 1700000000, "model": "sonar-pro",
			"choices": []map[string]any{{
...
func TestRegistration(t *testing.T) {
	...
	if !found {
		t.Error("perplexity provider not registered")
	}
}

func TestNew(t *testing.T) {
	m, err := New(config.ProviderConfig{Model: "sonar-pro", APIKey: "test"})
	if err != nil {
		t.Fatalf("New() error: %v", err)
	}
	if m.ModelID() != "sonar-pro" {
		t.Errorf("ModelID() = %q, want %q", m.ModelID(), "sonar-pro")
	}
}
...
func TestGenerate(t *testing.T) {
	ts := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		fmt.Fprint(w, mockResponse("Hello from Perplexity!"))
	}))
	defer ts.Close()

	m, _ := New(config.ProviderConfig{
		Model: "sonar-pro", APIKey: "test", BaseURL: ts.URL,
	})
	...
	if resp.Text() != "Hello from Perplexity!" {
		t.Errorf("text = %q, want %q", resp.Text(), "Hello from Perplexity!")
	}
}
...
func TestStream(t *testing.T) {
	ts := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		fmt.Fprint(w, streamResponse([]string{"Perplexity", " AI"}))
	}))
	defer ts.Close()
	...
	if text.String() != "Perplexity AI" {
		t.Errorf("text = %q, want %q", text.String(), "Perplexity AI")
	}
}

func TestBindTools(t *testing.T) {
	m, _ := New(config.ProviderConfig{Model: "sonar-pro", APIKey: "test"})
	bound := m.BindTools([]schema.ToolDefinition{
		{Name: "search", Description: "search"},
	})
	if bound.ModelID() != "sonar-pro" {
		t.Errorf("ModelID = %q", bound.ModelID())
	}
}

func TestRegistryNew(t *testing.T) {
	m, err := llm.New("perplexity", config.ProviderConfig{
		Model: "sonar-pro", APIKey: "test",
	})
	if err != nil {
...
func TestDefaultBaseURL(t *testing.T) {
	if defaultBaseURL != "https://api.perplexity.ai" {
