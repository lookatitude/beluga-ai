...
func mockResponse(content string) string {
	resp := map[string]any{
		"id": "chatcmpl-qwen", "object": "chat.completion",
		"created": 1700000000, "model": "qwen-plus",
...
func streamResponse(deltas []string) string {
	...
	for _, d := range deltas {
		chunk := map[string]any{
			"id": "chatcmpl-qwen", "object": "chat.completion.chunk",
			"created": 1700000000, "model": "qwen-plus",
			"choices": []map[string]any{{
...
func TestRegistration(t *testing.T) {
	...
	if !found {
		t.Error("qwen provider not registered")
	}
}

func TestNew(t *testing.T) {
	m, err := New(config.ProviderConfig{Model: "qwen-plus", APIKey: "test"})
	if err != nil {
		t.Fatalf("New() error: %v", err)
	}
	if m.ModelID() != "qwen-plus" {
		t.Errorf("ModelID() = %q, want %q", m.ModelID(), "qwen-plus")
	}
}
...
func TestGenerate(t *testing.T) {
	ts := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		fmt.Fprint(w, mockResponse("Hello from Qwen!"))
	}))
	defer ts.Close()

	m, _ := New(config.ProviderConfig{
		Model: "qwen-plus", APIKey: "test", BaseURL: ts.URL,
	})
	...
	if resp.Text() != "Hello from Qwen!" {
		t.Errorf("text = %q, want %q", resp.Text(), "Hello from Qwen!")
	}
}
...
func TestStream(t *testing.T) {
	ts := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		fmt.Fprint(w, streamResponse([]string{"Qwen", " AI"}))
	}))
	defer ts.Close()
	...
	if text.String() != "Qwen AI" {
		t.Errorf("text = %q, want %q", text.String(), "Qwen AI")
	}
}

func TestBindTools(t *testing.T) {
	m, _ := New(config.ProviderConfig{Model: "qwen-plus", APIKey: "test"})
	bound := m.BindTools([]schema.ToolDefinition{
		{Name: "search", Description: "search"},
	})
	if bound.ModelID() != "qwen-plus" {
		t.Errorf("ModelID = %q", bound.ModelID())
	}
}

func TestRegistryNew(t *testing.T) {
	m, err := llm.New("qwen", config.ProviderConfig{
		Model: "qwen-plus", APIKey: "test",
	})
	if err != nil {
...
func TestDefaultBaseURL(t *testing.T) {
	if defaultBaseURL != "https://dashscope.aliyuncs.com/compatible-mode/v1" {
