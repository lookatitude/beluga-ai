...
func mockResponse(content string) string {
	resp := map[string]any{
		"id": "chatcmpl-xai", "object": "chat.completion",
		"created": 1700000000, "model": "grok-3",
...
func streamResponse(deltas []string) string {
	...
	for _, d := range deltas {
		chunk := map[string]any{
			"id": "chatcmpl-xai", "object": "chat.completion.chunk",
			"created": 1700000000, "model": "grok-3",
			"choices": []map[string]any{{
...
func TestRegistration(t *testing.T) {
	...
	if !found {
		t.Error("xai provider not registered")
	}
}
...
func TestNew(t *testing.T) {
	...
	if m.ModelID() != "grok-3" {
		t.Errorf("ModelID() = %q, want %q", m.ModelID(), "grok-3")
	}
}

func TestNew_CustomModel(t *testing.T) {
	m, err := New(config.ProviderConfig{Model: "grok-3-mini", APIKey: "test"})
	if err != nil {
		t.Fatalf("New() error: %v", err)
	}
	if m.ModelID() != "grok-3-mini" {
		t.Errorf("ModelID() = %q, want %q", m.ModelID(), "grok-3-mini")
	}
}
...
func TestGenerate(t *testing.T) {
	ts := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		fmt.Fprint(w, mockResponse("Hello from Grok!"))
	}))
	defer ts.Close()

	m, _ := New(config.ProviderConfig{
		Model: "grok-3", APIKey: "test", BaseURL: ts.URL,
	})
	...
	if resp.Text() != "Hello from Grok!" {
		t.Errorf("text = %q, want %q", resp.Text(), "Hello from Grok!")
	}
}
...
func TestStream(t *testing.T) {
	ts := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		fmt.Fprint(w, streamResponse([]string{"Grok", " says", " hi"}))
	}))
	defer ts.Close()
	...
	if text.String() != "Grok says hi" {
		t.Errorf("text = %q, want %q", text.String(), "Grok says hi")
	}
}

func TestBindTools(t *testing.T) {
	m, _ := New(config.ProviderConfig{Model: "grok-3", APIKey: "test"})
	bound := m.BindTools([]schema.ToolDefinition{
		{Name: "search", Description: "search the web"},
	})
	if bound.ModelID() != "grok-3" {
		t.Errorf("ModelID = %q", bound.ModelID())
	}
}

func TestRegistryNew(t *testing.T) {
	m, err := llm.New("xai", config.ProviderConfig{
		Model: "grok-3", APIKey: "test",
	})
	if err != nil {
...
func TestGenerate_ToolCalls(t *testing.T) {
	...
	if ai.ToolCalls[0].Name != "calc" {
		t.Errorf("ToolCall name = %q, want %q", ai.ToolCalls[0].Name, "calc")
	}
}
