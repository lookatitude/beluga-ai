...
// Config holds configuration for the Memgraph GraphStore.
type Config struct {
	// URI is the Memgraph connection URI (e.g., "bolt://localhost:7687").
	URI string
...
// sessionRunner abstracts Memgraph session operations for testability.
...
// memgraphRunner wraps a real Neo4j/Memgraph driver.
type memgraphRunner struct {
	drv      neo4j.DriverWithContext
	database string
}

func (r *memgraphRunner) executeWrite(ctx context.Context, cypher string, params map[string]any) error {
	session := r.drv.NewSession(ctx, neo4j.SessionConfig{DatabaseName: r.database})
...
func (r *memgraphRunner) executeRead(ctx context.Context, cypher string, params map[string]any) ([]record, error) {
	session := r.drv.NewSession(ctx, neo4j.SessionConfig{
...
func (r *memgraphRunner) close(ctx context.Context) error {
	return r.drv.Close(ctx)
}

// nodeWrapper holds data extracted from a Memgraph node.
type nodeWrapper struct {
	elementID string
	props     map[string]any
}

// relWrapper holds data extracted from a Memgraph relationship.
...
// GraphStore is a Memgraph-backed implementation of memory.GraphStore.
// It uses Cypher queries for all graph operations via the Bolt protocol.
type GraphStore struct {
	runner sessionRunner
}

// New creates a new Memgraph GraphStore with the given configuration.
...
// nodes with properties set from the entity's Properties map.
...
// compatible values (strings, numbers, booleans). Nil maps are returned
// as empty maps to avoid Cypher SET errors.
