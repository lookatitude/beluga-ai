...
func New(cfg Config) (*MessageStore, error) {
	if cfg.Client == nil {
		return nil, core.Errorf(core.ErrInvalidInput, "redis: client is required")
	}
	key := cfg.Key
	if key == "" {
		key = "beluga:messages"
...
func (s *MessageStore) Append(ctx context.Context, msg schema.Message) error {
	...
	if err != nil {
		return core.Errorf(core.ErrInvalidInput, "redis: marshal message: %w", err)
	}
...
func (s *MessageStore) Search(ctx context.Context, query string, k int) ([]schema.Message, error) {
	...
	if err != nil {
		return nil, core.Errorf(core.ErrProviderDown, "redis: search: %w", err)
	}
...
func (s *MessageStore) All(ctx context.Context) ([]schema.Message, error) {
	...
	if err != nil {
		return nil, core.Errorf(core.ErrProviderDown, "redis: all: %w", err)
	}
